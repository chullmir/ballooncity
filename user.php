		...
		<div class="row">
			...
			<div class="col-12 col-md-6">
				<div class="row registrarse">
					...
					<div class="col-12">
						<form class="form-group" method="post" >
							<input type="text" class="form-control form-name" value="<?= $FormData->getName() ?>" placeholder="Nombre" name="name" id="name" >
							<p class="error"><?php echo (($FormData->fieldHasError('name') ? $FormData->getFieldErrors('name') : '')); ?></p>


							<input type="text" class="form-control form-email" value="<?= $FormData->getEmail()?>" placeholder="Email" name="email" id="email">
							<p class="error"><?php echo (($FormData->fieldHasError('email') ? $FormData->getFieldErrors('email') : '')); ?></p>
							<input type="password" class="form-control form-password" placeholder="Contraseña" name="password" id=
							"password">
							<p class="error"><?php echo (($FormData->fieldHasError('password') ? $FormData->getFieldErrors('password') : '')); ?></p>
